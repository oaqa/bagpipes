...
public class FileSystemCollectionReader extends JCasCollectionReader_ImplBase {
	...
	public void initialize(UimaContext uimaContext) {
		System.out.println(PARAM_INPUTDIR + ": "
				+ getConfigParameterValue(PARAM_INPUTDIR));
		// String filePath = ((String)
		// getConfigParameterValue(PARAM_INPUTDIR)).trim();
		// InputStream iStream = Files.newDirectoryStream(new Path(filePath));
		File directory = new File(
				((String) getConfigParameterValue(PARAM_INPUTDIR)).trim());
		// StreamUtil.stream2file(iStream);
