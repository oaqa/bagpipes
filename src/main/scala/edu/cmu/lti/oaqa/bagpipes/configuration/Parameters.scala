...
object Parameters {
  ...
  case class ListParameter(pList: List[Parameter]) extends Parameter(pList.map(p => p.getElem))
  case class MapParameter(map: Map[String, Parameter]) extends Parameter(map.map { case (k, v) => (k -> v.getElem) })
